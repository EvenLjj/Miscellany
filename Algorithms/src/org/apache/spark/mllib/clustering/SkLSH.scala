...
class SkLSH(private var k:Int,
            ...
            private var subpartial: Double) extends KMeans with Serializable{
  ...
  def SpectralDimReduction(data: RDD[Vector], nParts: Int): RDD[(Long,(Vector, Vector))] = {
    ...
    val parseddata = data.zipWithIndex().partitionBy(new HashPartitioner(nParts)).cache()
   ...
   val M = math.max((math.log(numDims)/math.log(2)).ceil.toInt,20)
    ...
    val lsh = new com.liu.bigds.clustering.hash.LSH(parseddata, hyperplanes, thresholds, M)
    val model = lsh.run()
